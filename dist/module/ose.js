...
class OseActor extends Actor {
  ...
  computeModifiers() {
    ...
    const data = this.data.data;
	const modTbl = game.settings.get("ose", "whiteBoxMods");
...
const capped = {
	...
	if (modTbl === true) {
		...
		data.scores.dex.init = 0;
		data.scores.cha.npc = data.scores.cha.mod;
		data.scores.cha.retain = OseActor._valueFromTable(standard, data.scores.cha.value) + 4;
		data.scores.cha.loyalty = OseActor._valueFromTable(whiteboxLoyal, data.scores.cha.value);
		data.exploration.odMod = 3;
	} else {
		data.scores.str.mod = OseActor._valueFromTable(
		  standard,
		  data.scores.str.value
...
const registerSettings = function () {
  ...
  game.settings.register("ose", "whiteBoxMods", {
    name: game.i18n.localize("OSE.Setting.WhiteBoxMods"),
    hint: game.i18n.localize("OSE.Setting.WhiteBoxModsHint"),
    default: false,
    scope: "world",
    type: Boolean,
    config: true,
